...
evcnt2 <- stormsub %>%
        select(STATE,EVCLASS,EVTYPE,FATALITIES,INJURIES,PROPDMG,CROPDMG,EV_TIMESTAMP) %>%
...
evcnt3 <- stormsub %>%
        ...
        summarise(ECONLOSS = (sum(PROPDMGS) + sum(CROPDMGS)) / 1e+09,
                  CASULTIE = (sum(FATALITIES) + sum(INJURIES)) / 1e+03)

par(mai = c(0.25, 1.10, 0.25, 1.10), cex.axis = 0.75, ann = FALSE, yaxt = "s")

barplot(CASULTIE ~ EVCLASS + STATE, data = evcnt3,
        border = NA,
        col = pal)
...
evcnt4 <- stormsub %>% 
        select(EV_TIMESTAMP, EVCLASS, FATALITIES,INJURIES, PROPDMGS,CROPDMGS) %>%
        arrange(EV_TIMESTAMP) %>%
        group_by(YEARS = year(EV_TIMESTAMP), EVCLASS) %>%
        summarise(EVCNT = n(),
                  CASUALTI = sum(FATALITIES + INJURIES),
                  ECONLOSS = mean(PROPDMGS + CROPDMGS))
