...
evclass.rowid <- grep("SUMMARY", StormData$EVTYPE)
evclass.label <- ifelse(length(evclass.rowid) > 0, "EXCLUDED", "")
evclass.evtyp <- StormData[evclass.rowid,]$EVTYPE
evclass <- cbind(evclass.rowid, evclass.evtyp)
StormData <- StormData %>% 
        mutate(EVCLASS = replace(EVCLASS, evclass.rowid, evclass.label))
