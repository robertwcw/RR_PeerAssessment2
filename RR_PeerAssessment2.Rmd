...
barplot(evcnt$EVCNT, horiz = TRUE, border = NA, col = pal, names.arg = unique(evcnt$EVCLASS))
...
m <- barplot(evcnt$EVCNT, horiz = TRUE, plot = FALSE)
for (i in 1:length(m)) {
        j <- ifelse(evcnt$EVCNT[i] >= maxtik, -2.25, 0.25)
        text(evcnt$EVCNT[i], m[i], labels = formatC(evcnt$EVCNT[i], big.mark = ","), pos = 4,  offset = j, cex = 0.70)
}
...
# most hazardous weather events in terms of casualty count and economic loss
...
evcnt <- stormsub %>%
        ...
        summarise(FATALCNT = sum(FATALITIES),
                  ...
                  ECONLOSS = ((PROPDMGS + CROPDMGS) / 1e+09),
                  CASULTIE = ((FATALCNT + INJURCNT) / 1e+03))

par(mai = c(0.50, 1.15, 0.25, 1.00), cex.axis = 0.50)
barplot(CASULTIE ~ EVCLASS + as.character(STATE), data = evcnt,
        border = NA,
        col = pal)
title(ylab = "Casualty Count ('000)", cex.lab = 0.70)
...
barplot(ECONLOSS ~ EVCLASS + STATE, data = evcnt,
        border = NA,
        col = pal)
...
title(ylab = "Event Frequency by Type ('000)")
mtext("Weather Events Frequency Aggregate of Individual-State (1950 ~ 2011)", font = 2, side = 3, line = 0.25, adj = 0, padj = 0, cex = 0.80)
mtext("Manifested in Casualty & Property/Crop Loss", font = 2, side = 3, line = -0.75, adj = 0, padj = 0, cex = 0.80)
```
