...
Structure of the **`r filenam[1]`** data set.

```{r eda.1, tidy=TRUE}
str(get(filenam[1]))
```
&nbsp;
<!-- There are **`r length(names(get(filenam[1])))`** columns found in the **`r filenam[1]`** data set.   -->
&nbsp;

Preliminary visual examination on the `r filenam[1]` data set revealed there are a number of data columns in the `r class(get(filenam[1]))[1]` entail further processing and transformation that are compulsory to enable downstream analytic processing, as understood the **`r filenam[1]`** data set, is quite "dirty".  
&nbsp;

## Data Processing

For the purpose of this analysis work, which is to answer the pertinent questions about the weather events of WHERE, WHEN and WHAT; we will proceed to spawn a new data table object called **stormsub** with a subset of 11 variables from existing raw data set `r filenam[1]`, plus a few more derived variables base on existing vectors. The following is the subset of variables grabbed from the existing raw data set **`r filenam[1]**`:  
...
**EV_TIMESTAMP** *< derived-variable >*  
Generate a time-stamp in POSIXct type, base on existing raw data BGN_DATE & BGN_TIME for each event as it was recorded in the raw data set. 
...
**LATITUDE** & **LONGITUDE**  
Geographic coordinates are used in mapping weather events onto the map of USA to visually tell user where were the events took place. **LATITUDE** and **LONGITUDE** data in their current form are not use-able by any mapping system, must go through a conversion process to transform them into geographic coordinate system configuration to be readily recognized by base R mapping tools.
...
**EVTYPE**  
Data values captured in EVTYPE in its raw format is quite a bit "dirty" and present itself in human natural language construct than conforming to format useful for information processing using computers. To enable simpler data processing logic coding and avoid unnecessary error during analytic processing run, we trim un-needed white-spaces from within **EVTYPE** and then convert to upper case. Please refer to **EVCLASS** for further explanation.
...
**PROPDMGEXP**  
**CROPDMGEXP**  
Trim un-needed embedded white-spaces, if any, from the data values in these data columns and convert to upper case words, to enable simpler analytic processing logic coding.
```{r data.proc.4, tidy=TRUE}
StormData$STATE <- toupper(trimws(StormData$STATE))
StormData$COUNTY <- toupper(trimws(StormData$COUNTY))
StormData$PROPDMGEXP <- toupper(trimws(StormData$PROPDMGEXP))
StormData$CROPDMGEXP <- toupper(trimws(StormData$CROPDMGEXP))
```
&nbsp;

**EVCLASS** *< derived-variable >*  
Re-classification of weather events type, to standardize weather event type coding to ensure they are in conformity to the requirement of computer information processing that shall improved reporting.
...
**PROPDMGS** *< derived-variable >*  
Amount of property damages derived from **PROPDMG** augmented by **PROPDMGEXP** number scale.
&nbsp;

**CROPDMGS** *< derived-variable >*  
Amount of crop damages derived from  **CROPDMG** augmented by **PROPDMGEXP** number scale.
&nbsp;

**stormsub**  
Comprising a subset of the weather event data spawned from the raw data set **StormData**, institute the basis of all subsequent analytic processing. 

Please take note, off-shore affiliated territories of the USA are protectorates not under direct administration of the USA, therefore shall be excluded from **stormsub** data table.
...
```{r data.analytic, tidy=TRUE, fig.width=13, fig.height=12, fig.cap="Figure-1 : Weather Events Overview across USA"} 
par(par.def)
...
mtext("Property/Crop Loss by Event Type of Individual-State (1950 ~ 2011)", font = 2, side = 3, line = -3.00, adj = 1, padj = 0, cex = 0.75)
```
&nbsp;

Graph plot in Figure-1 above illustrates the overview of weather events across the USA from 1950 to 2011.

Qeustion 1.  
Which types of events are most harmful with respect to population health?

**(1)Tornado**, **(2)Storm**, **(3)Flood** and **(4)Hail** are most hazardous with respect to population health in the order of casualty count of each event type.

Question 2.
Which types of events have the greatest economic consequences?

**(1)Storm**, **(2)Flood** and **(3)Tornado** are most devastated in terms of economic consequences in the order of property/crops loss caused by each event type.

Conclusion: Not in any particular order, **Tornado**, **Storm** and **Flood** are most hazardous weather in the USA.

&nbsp;

<!-- trends of weather events across the usa from 1950 ~ 2011 -->
```{r data.analytic2, tidy=TRUE, results="hold", fig.width=12, fig.height=8, fig.cap="Figure-2 : Trends of Weather Events"}
par(par.def)
...
From Figure-2 above, the frequency of occurrences of weather event are generally on the up trend since 1950. Closer inspection on the graph indicates incidence data of some event types e.g. **Extreme.Temp**, **Snow.Ice**, **Rain.Flood**, **Wild.Fire** and **Lightning** only began in 1993. However, does not invalidate earlier consensus that the rate of occurrences of the measured weather events are picking up in frequency and getting more rapid in recent years.  
&nbsp;

<!-- maps of the usa with weather events mapping -->
```{r data.analytic3, tidy=TRUE, results="hold", fig.width=12, fig.height=8, fig.cap="Figure 3 : Locale of Most Hazardous Weather Events (1990 ~ 2010)"}
...
points(x = ev.w.gcs$LONGITUDE, y = ev.w.gcs$LATITUDE, pch = 20, cex = 0.5, 
...
plot(log(ECONLOSS.sum) ~ YEARS, 
     data = subset(evcnt4, as.numeric(EVCLASS) %in% ev.klass),
     pch = 19, col = pal[ev.klass], cex = 0.8)
title(main = "Property/Crop Loss", ylab = "(log scale)")
```
&nbsp;

The map of USA in Figure-3 illustrates the locale of the 3-most hazardous weather type in the USA for period 1990 ~ 2010.  

Scatter plot graphs for Property/Crop Loss and Number of Casualty at The bottom of Figure-3 yield some interesting facts about their opposing trends; Property/Crop Loss is rising and Number of Casualty is declining. As the frequency of occurrences of weather events across the USA is increasing, stationary assets such as crops and properties will suffer damages when the events strike but human will stay behind shelters to evade the disaster.
&nbsp;
