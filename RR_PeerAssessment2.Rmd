...
**LONGITUDE** & **LATITUDE**  
The geographic coordinates (x,y) used in mapping weather events onto the map of USA to visually tell user where were the events took place. **LATITUDE** and **LONGITUDE** data in their current form are not immediately use-able by any mapping system, must go through a conversion process to transform them into geographic coordinate system configuration to be readily recognized by base R mapping tools or any other mapping systems.
...
**PROPDMGEXP**  
**CROPDMGEXP**  
Trim un-needed embedded white-spaces, if any, in these data vectors and convert them to upper case words, again for conformity.
...
Re-classification of weather event type, to standardize type coding to overcome the non-standardize text format present in **EVTYPE**, to ensure they are in conformity with the requirement of analytic processing.
...
Comprise of subset of the weather event data spawned from the raw data set **StormData**, instituting the basis of all subsequent analytic processing and reporting. 
...
```
Please take note, off-shore affiliated territories of the USA are protectorates not under direct administration of the USA, therefore shall be excluded from **stormsub** data set.  
&nbsp;
&nbsp;
...
Barchart in Figure 1 above illustrates the overview of weather events across the USA from 1950 to 2011.
...
**(1)Storm**, **(2)Flood** and **(3)Tornado** are most devastated in terms of economic consequences in the order of amount of property/crops damaged by each event type.
...
```{r data.analytic2, message=TRUE, tidy=TRUE, fig.width=12, fig.height=8, fig.cap="Figure 2: Trends of Weather Events"}
par(par.def)
...
From scatter plot in  above, the frequency of occurrence of weather event are generally on the up trend since 1950 till 2011. Closer inspection on the scatter plot indicates incidence data of some event types e.g. **Extreme.Temp**, **Snow.Ice**, **Rain.Flood**, **Wild.Fire** and **Lightning** only began in 1993. According to National Weather Service https://www.weather.gov/timeline, in 1989 an eight year plan was announced for the modernization and restructuring of NWS agency. The massive overhaul change the way the agency operates resulting in improved capabilities, beginning 1993 enable more observation dimensions hence more data collected. However, different length of observation does not invalidate earlier consensus that the rate of occurrence of all measured weather events are trending in the sam direction and getting more rapid.  
&nbsp;
...
The map of USA in  illustrates the locale of the 3-most hazardous weather type in the USA for period 1990 ~ 2010.  

Scatter plot graphs for Property/Crop Loss and Number of Casualty in The bottom of \ref{fig:figs} yield an interesting phenomenon about their opposing trends; Property/Crop Loss is rising while Number of Casualty is declining. As the frequency of occurrences of weather events across the USA is increasing, stationary assets such as crops and properties will suffer damages when the events strike but human will stay behind shelters to evade the disaster.
&nbsp;
&nbsp;
