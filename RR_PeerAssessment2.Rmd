...
udf.expont <- function(x, y) {
        ifelse(x %in% c("1","2","3","4","5","6","7","8","9"), x <- paste0("e+0",x),
               ifelse(x == "H", x <- "e+02",
                      ifelse(x == "K", x <- "e+03",
                             ifelse(x == "M", x <- "e+06",
                                    ifelse(x == "B", x <- "e+09", x <- "e+0")))))
...
mtext("Weather Event Aggregate Frequency Across USA (1950 ~ 2011)", font = 2, side = 3, line = -1.25, adj = 1, cex = 0.70)
...
barplot(n ~ EVCLASS + as.character(STATE), data = evcnt, 
        ...
        border = NA,
        col = pal)
title(ylab = "Aggregate Frequency by Event Type")
mtext("Weather Events of Individual-State (1950 ~ 2011)", font = 2, side = 3, line = -1.25, adj = 0, padj = 0, cex = 0.70)
