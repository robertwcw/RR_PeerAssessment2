...
repeat {
        i <- i + 1
        i <- ifelse(i > 23, break, i)
        if (!is.na(ev.clvl[i,1])) {
                ev.clab <- c(ev.clab, ev.clvl[i,2])
        }
}
...
stormsub <- StormData %>% 
        ...
        filter(as.character(STATE) %in% states & !(EVCLASS %in% c("X","Z"))) %>% 
                # (!is.na(LONGITUDE) & !is.na(LATITUDE)) &
                #        (LONGITUDE != 0 & LATITUDE != 0)) %>%
        mutate(EVCLASS = factor(EVCLASS, labels = ev.clab)) %>%
        arrange(EV_TIMESTAMP)


pal <- pal[seq(length(pal), by = -1, length.out = length(evcnt$EVCLASS))]

# set up global graphics parameters for the plots
par(mfrow = c(1,2), las = 1, cex.axis = 0.65, ann = FALSE)
...
mtext("Aggregate Frequency of Weather Events in USA (1950 ~ 2011)", font = 2, side = 3, line = -1.25, adj = 1, cex = 0.75)
...
par(mai = c(0.75,0.15,0.50,1.15), yaxt = "n")
barplot(evcnt$n, horiz = TRUE, border = NA, col = pal, names.arg = unique(evcnt$EVCLASS))
...
mtext("Aggregate Frequency of Weather Events in USA ()1950 ~ 2011)", font = 2, side = 3, line = -1.25, adj = 1, cex = 0.75)
mtext("Manifested in Casualty & Economic Loss", font = 2, side = 3, line = -2.00, adj = 1, cex = 0.75)
mtext("(US Affiliated Territories Not Included)", side = 3, line = -2.75, adj = 1, cex = 0.65)


par(par.def)
par(mfrow = c(2,1), cex.axis = 0.60, cex.lab = 0.75, ann = FALSE)
...
barplot(n ~ EVCLASS + as.character(STATE), data = evcnt, 
        ...
        border = NA,
        col = pal)
mtext("(US Affiliated Territories Not Included)", side = 3, line = -1.25, adj = 0, cex = 0.65)
title(ylab = "Events Frequency of Individual-State")
...
# title(main = "Weather Events Aggregate Across The USA (1950 ~ 2011)", cex = 0.90)
